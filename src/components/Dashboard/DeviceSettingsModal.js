...
class DeviceSettingsModal extends Component {
  ...
  handleAliasChange(e) {
    this.setState({
      ...
      hasSavedAlias:
        e.target.value === this.props.device.dongle_id
          ? this.state.hasSavedAlias
          : false
    });
  }
  ...
  async shareDevice() {
    ...
    try {
      await Devices.grantDeviceReadPermission(
        dongle_id,
    ...
    return (
      ...
      >
        <Paper className={classes.modal}>
          <Typography variant="title">Edit Device</Typography>
          <Divider />
          <div className={classes.form}>
            <div className={classes.formRow}>
              ...
              />
              {(this.props.device.alias !== this.state.deviceAlias ||
                this.state.hasSavedAlias) && (
                <div className={classes.wrapper}>
                  <IconButton variant="fab" onClick={this.setDeviceAlias}>
                    {(this.state.hasSavedAlias && <CheckIcon />) || (
                      <SaveIcon />
                    )}
                  </IconButton>
                  {this.state.loadingDeviceAlias && (
                    <CircularProgress
                      size={48}
