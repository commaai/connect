...
import { setDeviceAlias, grantDeviceReadPermission } from 'comma-api/src/devices';
...
class DeviceSettingsModal extends Component {
  ...
  async setDeviceAlias () {
    ...
    try {
      const device = await setDeviceAlias(dongle_id, this.state.deviceAlias.trim());
  ...
  async shareDevice () {
    ...
    try {
      await grantDeviceReadPermission(dongle_id, this.state.shareEmail.trim());
      this.setState({
        loadingDeviceShare: false,
        shareEmail: '',
        hasShared: true
