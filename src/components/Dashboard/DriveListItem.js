...
class DriveListDrive extends Component {
  ...
  componentDidMount() {
    ...
    this.mounted = true;
    GeocodeApi()
      .reverseLookup(drive.startCoord)
      .then(startLocation => {
        if (!this.mounted) {
          return;
        }
  ...
  render() {
    ...
    const endTime = fecha.format(
      new Date(drive.startTime + drive.duration + 1000),
      "HH:mm"
    );
        ...
        <div className={classes.driveHeader}>
          <Grid container>
            <Grid item xs={4} className={classes.driveHeaderIntro}>
              <div className={classes.driveAvatar}>{drive.annotations}</div>
              <div className={classes.driveTitle}>
                <Typography variant="body2">
                  {startDate} @ {startTime} to {endTime}
                </Typography>
                <Typography>{deviceAlias}</Typography>
              </div>
            </Grid>
            <Grid item xs={2}>
              <Typography variant="body2">
