...
<button type="button" class="button addButton {asClass}" style={asStyle} onclick={() => { modalOpen = true; }}>
      ...
      {:else}
        <div class="videoContainer {scrim ? 'videoScrim' : ''}">
          ...
          <div class="videoOverlay">
            ...
            {#if pairError}
              <p>{pairError}</p>
              <button type="button" class="button retryButton" onclick={restart}>
                <span class="label">try again</span>
              </button>
            {/if}
            {#if pairDongleId}
              <p>Successfully paired device <span class="font-bold">{pairDongleId}</span></p>
              <button type="button" class="button retryButton" onclick={modalClose}>
