import * as Sentry from '@sentry/browser';

import { athena as Athena, devices as Devices, raw as Raw } from '$lib/api';
import { asyncSleep, deviceOnCellular, deviceVersionAtLeast } from '$lib/utils';

export const FILE_NAMES = {
  qcameras: ['qcamera.ts'],
  cameras: ['fcamera.hevc'],
  dcameras: ['dcamera.hevc'],
  ecameras: ['ecamera.hevc'],
  qlogs: ['qlog.bz2', 'qlog.zst'],
  logs: ['rlog.bz2', 'rlog.zst'],
};

const MAX_OPEN_REQUESTS = 15;
const MAX_RETRIES = 5;

// connect uploads should be high priority as they are user requested (lower is higher)
const HIGH_PRIORITY = 0;

let uploadQueueTimeout = null;
let openRequests = 0;

/**
 * The redux `files` / `filesUploading` / `filesUploadingMeta` slices.
 *
 * `files` is keyed by `${dongleId}|${logId}--${segment}/${type}`, so entries for
 * different devices cannot collide and the map is kept flat, as the reducer had it.
 */
const filesState = $state({
  files: null,
  uploading: {},
  uploadingMeta: { dongleId: null, fetchedAt: null },
});

/**
 * Athena calls learn whether a device is online and metered as a side effect. Redux
 * merged that into the device records; the device list here comes from the layout
 * load, so keep the learned fields beside it and merge on read.
 */
const deviceStatus = $state({});

export function getFiles() {
  return filesState.files;
}

export function getFile(fileName) {
  return filesState.files?.[fileName];
}

export function getUploadQueue() {
  return filesState.uploading;
}

export function getUploadQueueMeta() {
  return filesState.uploadingMeta;
}

/** A device record with whatever athena has since reported about it merged in. */
export function withDeviceStatus(device) {
  const status = device && deviceStatus[device.dongle_id];
  return status ? { ...device, ...status } : device;
}

function updateDeviceOnline(dongleId, lastAthenaPing) {
  deviceStatus[dongleId] = {
    ...deviceStatus[dongleId],
    last_athena_ping: lastAthenaPing,
    fetched_at: Math.floor(Date.now() / 1000),
  };
}

function updateDeviceNetwork(dongleId, networkMetered) {
  deviceStatus[dongleId] = {
    ...deviceStatus[dongleId],
    network_metered: networkMetered,
  };
}

function pathToFileName(dongleId, path) {
  const [seg, fileType] = path.split('/');
  const type = Object.entries(FILE_NAMES).find((e) => e[1].includes(fileType))[0];
  return `${dongleId}|${seg}/${type}`;
}

async function athenaCall(dongleId, payload, sentryFingerprint, retryCount = 0) {
  try {
    while (openRequests > MAX_OPEN_REQUESTS) {
      // eslint-disable-next-line no-await-in-loop
      await asyncSleep(2000);
    }
    openRequests += 1;
    const resp = await Athena.postJsonRpcPayload(dongleId, payload);
    openRequests -= 1;
    return resp;
  } catch (err) {
    openRequests -= 1;
    if (!err.resp && retryCount < MAX_RETRIES) {
      await asyncSleep(2000);
      return athenaCall(dongleId, payload, sentryFingerprint, retryCount + 1);
    }
    if (err.message && (err.message.indexOf('Timed out') === -1
      || err.message.indexOf('Device not registered') === -1)) {
      return { offline: true };
    }
    console.error(err);
    Sentry.captureException(err, { fingerprint: sentryFingerprint });
    return { error: err.message };
  }
}

/**
 * Ported from actions/index.js, which owned it only because the device slice lived
 * there. It exists for the upload queue, which needs `network_metered` to know
 * whether uploads are waiting for wifi.
 */
async function fetchDeviceNetworkStatus(dongleId, device) {
  const metersDirectly = deviceVersionAtLeast(device, '0.8.14');
  const payload = {
    id: 0,
    jsonrpc: '2.0',
    method: metersDirectly ? 'getNetworkMetered' : 'getNetworkType',
  };
  try {
    const resp = await Athena.postJsonRpcPayload(dongleId, payload);
    if (resp && resp.result !== undefined) {
      // getNetworkType returns 1 for wifi and 6 for ethernet
      updateDeviceNetwork(dongleId, metersDirectly ? resp.result : (resp.result !== 1 && resp.result !== 6));
      updateDeviceOnline(dongleId, Math.floor(Date.now() / 1000));
    }
  } catch (err) {
    if (err.message && (err.message.indexOf('Timed out') === -1
      || err.message.indexOf('Device not registered') === -1)) {
      updateDeviceOnline(dongleId, 0);
    } else {
      console.error(err);
      Sentry.captureException(err, {
        fingerprint: metersDirectly ? 'athena_fetch_networkmetered' : 'athena_fetch_networktype',
      });
    }
  }
}

export async function setRouteViewed(dongleId, route, device) {
  if (!deviceVersionAtLeast(device, '0.9.6')) {
    return;
  }

  const payload = {
    id: 0,
    jsonrpc: '2.0',
    method: 'setRouteViewed',
    params: { route },
  };
  await athenaCall(dongleId, payload, 'action_files_set_route_viewed');
}

export async function fetchUploadUrls(dongleId, paths) {
  try {
    const resp = await Raw.getUploadUrls(dongleId, paths, 7);
    if (resp && !resp.error) {
      return resp.map((r) => r.url);
    }
  } catch (err) {
    console.error(err);
    Sentry.captureException(err, { fingerprint: 'action_files_upload_geturls' });
  }
  return null;
}

export function updateFiles(files) {
  filesState.files = { ...filesState.files, ...files };
}

export async function fetchFiles(routeName, nocache = false) {
  let files;
  try {
    files = await Raw.getRouteFiles(routeName, nocache);
  } catch (err) {
    console.error(err);
    Sentry.captureException(err, { fingerprint: 'action_files_fetch_files' });
    return;
  }

  // the API returns absolute urls, and the only place the segment number survives is
  // the path after the `${dongleId}/${logId}` prefix
  const urlName = routeName.replace('|', '/');
  const urls = Object
    .keys(FILE_NAMES)
    .filter((type) => files[type])
    .flatMap((type) => files[type].map((file) => ([type, file])))
    .reduce((state, [type, file]) => {
      const segmentNum = parseInt(file.split(urlName)[1].split('/')[1], 10);
      const fileName = `${routeName}--${segmentNum}/${type}`;
      state[fileName] = {
        url: file,
      };
      return state;
    }, {});

  filesState.files = { ...filesState.files, ...urls };
}

export function cancelFetchUploadQueue() {
  if (uploadQueueTimeout) {
    if (uploadQueueTimeout !== true) {
      clearTimeout(uploadQueueTimeout);
    }
    uploadQueueTimeout = null;
  }
}

/**
 * Poll athena's upload queue every 2s while there is something in it.
 *
 * `uploadQueueTimeout` doubles as the "a fetch is in flight" flag, so calling this
 * again while one is running is a no-op; stop it with cancelFetchUploadQueue().
 *
 * @param {object} device
 * @param {string} [currentDongleId] the dongle the app is looking at, which decides
 *   whether a finished upload is worth refetching route files for
 */
export async function fetchUploadQueue(device, currentDongleId = null) {
  if (uploadQueueTimeout) {
    return;
  }
  uploadQueueTimeout = true;

  const dongleId = device.dongle_id;
  fetchDeviceNetworkStatus(dongleId, device);

  const payload = {
    method: 'listUploadQueue',
    jsonrpc: '2.0',
    id: 0,
  };
  const uploadQueue = await athenaCall(dongleId, payload, 'action_files_athena_uploadqueue');
  if (!uploadQueue || !uploadQueue.result) {
    if (uploadQueue && uploadQueue.offline) {
      updateDeviceOnline(dongleId, 0);
    }
    cancelFetchUploadQueue();
    return;
  }
  updateDeviceOnline(dongleId, Math.floor(Date.now() / 1000));

  const prevFilesUploading = { ...filesState.uploading };
  const onCellular = deviceOnCellular(withDeviceStatus(device));
  const uploadingFiles = {};
  const newCurrentUploading = {};
  uploadQueue.result.forEach((uploading) => {
    const urlParts = uploading.url.split('?')[0].split('/');
    const filename = urlParts[urlParts.length - 1];
    const segNum = urlParts[urlParts.length - 2];
    const datetime = urlParts[urlParts.length - 3];
    const dongle = urlParts[urlParts.length - 4];
    const type = Object.entries(FILE_NAMES).find((e) => e[1].includes(filename))[0];
    const fileName = `${dongle}|${datetime}--${segNum}/${type}`;
    const waitingWifi = Boolean(onCellular && uploading.allow_cellular === false);
    uploadingFiles[fileName] = {
      current: uploading.current,
      progress: uploading.progress,
      paused: waitingWifi,
    };
    newCurrentUploading[uploading.id] = {
      fileName,
      current: uploading.current,
      progress: uploading.progress,
      createdAt: uploading.created_at,
      paused: waitingWifi,
    };
    delete prevFilesUploading[uploading.id];
  });
  // some item is done uploading
  if (currentDongleId === dongleId && Object.keys(prevFilesUploading).length) {
    const routeName = Object.values(prevFilesUploading)[0].fileName.split('--').slice(0, 2).join('--');
    fetchFiles(routeName, true);
  }
  filesState.uploading = newCurrentUploading;
  filesState.uploadingMeta = { dongleId, fetchedAt: Date.now() };
  if (Object.keys(uploadingFiles).length) {
    filesState.files = { ...filesState.files, ...uploadingFiles };
  }
  if (uploadQueueTimeout === true && uploadQueue.result.length) {
    cancelFetchUploadQueue();
    uploadQueueTimeout = setTimeout(() => {
      uploadQueueTimeout = null;
      fetchUploadQueue(device, currentDongleId);
    }, 2000);
  }
}

export async function doUpload(device, paths, urls) {
  const dongleId = device.dongle_id;
  let loopedUploads = !deviceVersionAtLeast(device, '0.8.13');
  if (!loopedUploads) {
    const filesData = paths.map((path, i) => ({
      fn: path,
      url: urls[i],
      headers: { 'x-ms-blob-type': 'BlockBlob' },
      allow_cellular: false,
      priority: HIGH_PRIORITY,
    }));
    const payload = {
      id: 0,
      jsonrpc: '2.0',
      method: 'uploadFilesToUrls',
      params: { files_data: filesData },
      expiry: Math.floor(Date.now() / 1000) + (86400 * 7),
    };
    const resp = await athenaCall(dongleId, payload, 'action_files_athena_uploads');
    if (resp && resp.error && resp.error.code === -32000
      && resp.error.data.message === 'too many values to unpack (expected 3)') {
      loopedUploads = true;
    } else if (!resp || resp.error) {
      const newUploading = paths.reduce((state, path) => {
        state[pathToFileName(dongleId, path)] = {};
        return state;
      }, {});
      updateDeviceOnline(dongleId, Math.floor(Date.now() / 1000));
      updateFiles(newUploading);
    } else if (resp.offline) {
      updateDeviceOnline(dongleId, 0);
    } else if (resp.result === 'Device offline, message queued') {
      const newUploading = paths.reduce((state, path) => {
        state[pathToFileName(dongleId, path)] = { progress: 0, current: false };
        return state;
      }, {});
      updateFiles(newUploading);
    } else if (resp.result) {
      const failed = resp.result.failed || [];

      // only if all file names for a segment file type failed
      const failedFiltered = [];
      for (const f of failed) {
        const failedCnt = failed.filter((p) => pathToFileName(dongleId, p) === pathToFileName(dongleId, f)).length;
        const requestedCnt = paths.filter((p) => pathToFileName(dongleId, p) === pathToFileName(dongleId, f)).length;
        if (failedCnt >= requestedCnt) {
          failedFiltered.push(f);
        }
      }

      if (failedFiltered) {
        const uploading = failedFiltered
          .reduce((state, path) => {
            const fn = pathToFileName(dongleId, path);
            state[fn] = { notFound: true };
            return state;
          }, {});
        updateFiles(uploading);
      }
      fetchUploadQueue(device, dongleId);
    }
  }

  if (loopedUploads) {
    for (let i = 0; i < paths.length; i++) {
      const payload = {
        id: 0,
        jsonrpc: '2.0',
        method: 'uploadFileToUrl',
        params: [paths[i], urls[i], { 'x-ms-blob-type': 'BlockBlob' }],
        expiry: Math.floor(Date.now() / 1000) + (86400 * 7),
      };
      // eslint-disable-next-line no-await-in-loop
      const resp = await athenaCall(dongleId, payload, 'files_actions_athena_upload');
      if (!resp || resp.error) {
        const uploading = {};
        uploading[pathToFileName(dongleId, paths[i])] = {};
        updateDeviceOnline(dongleId, Math.floor(Date.now() / 1000));
        updateFiles(uploading);
      } else if (resp.offline) {
        updateDeviceOnline(dongleId, 0);
      } else if (resp.result === 'Device offline, message queued') {
        const uploading = {};
        uploading[pathToFileName(dongleId, paths[i])] = { progress: 0, current: false };
        updateFiles(uploading);
      } else if (resp.result === 404 || resp?.result?.failed?.[0] === paths[i]) {
        const uploading = {};
        uploading[pathToFileName(dongleId, paths[i])] = { notFound: true };
        updateFiles(uploading);
      } else if (resp.result) {
        fetchUploadQueue(device, dongleId);
      }
    }
  }
}

export async function fetchAthenaQueue(dongleId) {
  let queue;
  try {
    queue = await Devices.getAthenaQueue(dongleId);
  } catch (err) {
    console.error(err);
    Sentry.captureException(err, { fingerprint: 'action_files_fetch_athena_queue' });
    return;
  }

  const newUploading = {};
  for (const q of queue) {
    if (!q.method || !q.expiry || q.expiry < Math.floor(Date.now() / 1000)) {
      continue;
    }

    if (q.method === 'uploadFileToUrl') {
      const fileName = pathToFileName(dongleId, q.params[0]);
      newUploading[fileName] = { progress: 0, current: false };
    } else if (q.method === 'uploadFilesToUrls') {
      for (const { fn } of q.params.files_data) {
        const fileName = pathToFileName(dongleId, fn);
        newUploading[fileName] = { progress: 0, current: false };
      }
    }
  }
  updateFiles(newUploading);
}

export async function cancelUploads(dongleId, ids) {
  const payload = {
    id: 0,
    jsonrpc: '2.0',
    method: 'cancelUpload',
    params: { upload_id: ids },
  };
  const resp = await athenaCall(dongleId, payload, 'action_files_athena_canceluploads');
  if (resp && resp.result && resp.result.success) {
    const idsArray = Array.isArray(ids) ? ids : [ids];
    const cancelFileNames = Object.keys(filesState.uploading)
      .filter((id) => idsArray.includes(id))
      .map((id) => filesState.uploading[id].fileName);
    if (filesState.files) {
      filesState.files = Object.keys(filesState.files)
        .filter((fileName) => !cancelFileNames.includes(fileName))
        .reduce((obj, fileName) => { obj[fileName] = filesState.files[fileName]; return obj; }, {});
    }
    filesState.uploading = Object.keys(filesState.uploading)
      .filter((id) => !idsArray.includes(id))
      .reduce((obj, id) => { obj[id] = filesState.uploading[id]; return obj; }, {});
  } else if (resp && resp.offline) {
    updateDeviceOnline(dongleId, 0);
  }
}
